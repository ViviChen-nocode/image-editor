...
const App: React.FC = () => {
  ...
  const [currentImage, setCurrentImage] = useState<string | null>(null); // The working copy
  
  // Cropper State (Global / Step 1)
  const [crop, setCrop] = useState<Crop>();
  const [completedCrop, setCompletedCrop] = useState<ReactImageCropPixel>();
  const [aspectRatio, setAspectRatio] = useState<number | undefined>(undefined); // undefined = free
  ...
  const [objectScale, setObjectScale] = useState<number>(1); // 1.0 = 100%
  ...
  const [newPresetName, setNewPresetName] = useState(''); // New State for input
  ...
  const handleUndo = () => {
    if (history.length > 1) {
        ...
        createImage(previousImage).then(img => {
             // Just ensure aspect ratio reference is correct
  ...
  // Triggered when the image inside ReactCrop loads
  const onImageLoad = (e: React.SyntheticEvent<HTMLImageElement>) => {
    const { width, height } = e.currentTarget;
    const initialAspect = aspectRatio || (16 / 9);
    
    // Only set initial crop if not already set (allows re-cropping to remember pos)
    if (!crop) {
  ...
  useEffect(() => {
    // Only update global aspect ratio crop if in step 1
    ...
    // Also handling Re-Crop aspect ratio update inside the render if needed,
    // but usually re-crop is free form or inherits the button setting.
  }, [aspectRatio, step]);
  ...
  const handleApplyCrop = async () => {
    ...
    if (sourceImage && completedCrop && imgRef.current) {
      try {
        ...
        pushHistory(croppedImage); // Add to history
  ...
  const handleConfirmReCrop = async () => {
      ...
      if (currentImage && completedCrop && reCropImgRef.current) {
         try {
            ...
            // Update aspect ratio reference for the new object
            const img = await createImage(croppedImage);
            setOriginalAspect(img.width / img.height);
  ...
  const applyPreset = (w: number, h: number) => {
      setMaintainAspect(false); // Unlink aspect to allow free preset size
      setDimensions({ width: w, height: h });
      // We don't change object scale/position immediately, let user decide
  };
  ...
  const deleteCustomPreset = (index: number, e: React.MouseEvent) => {
      // Prevent event bubbling to ensure parent click handlers don't trigger
      e.preventDefault();
      e.stopPropagation();
      
      // Removed window.confirm to avoid blocking issues in embedded environments
  ...
  // Helper to get current displayed bounds
  ...
  const handleScaleMouseDown = (e: React.MouseEvent | React.TouchEvent, position: string) => {
      ...
      scaleStartRef.current = {
          ...
          position // Store the handle position (e.g., 'top-left', 'bottom-right')
      };
  };
  ...
  const handleGlobalMove = (e: React.MouseEvent | React.TouchEvent) => {
      ...
      if (isScaling && scaleStartRef.current) {
          e.preventDefault();
          const clientY = 'touches' in e ? e.touches[0].clientY : (e as React.MouseEvent).clientY;
          let deltaY = scaleStartRef.current.clientY - clientY; // Up is positive (grow), Down is negative (shrink)
          
          // Logic Adjustment:
          // For TOP handles: Moving UP (deltaY > 0) = Grow. Moving DOWN (deltaY < 0) = Shrink. (Logic is Correct)
          // For BOTTOM handles: Moving DOWN (deltaY < 0) should be Grow. Moving UP (deltaY > 0) should be Shrink.
          // Therefore, we invert deltaY if dragging a bottom handle.
  ...
  const handleDownload = async () => {
      ...
      if (exportSettings.maxSizeKB) {
          ...
          // Flatten composition again just to be safe (though handleApplyResize did it)
  ...
  const renderEditor = () => {
    // Dynamic styles for the preview object
    const bounds = getObjectBounds();
    ...
    >
       ...
       <div className="lg:col-span-2 flex flex-col gap-4">
                             ...
                             <button 
                                onClick={() => {
                                    ...
                                    if(reCropImgRef.current) {
                                        ...
                                        setCrop(centerAspectCrop(width, height, 16/9)); // Default aspect or free
                                    } else {
                                        setCrop(undefined); // Will trigger onload
                                    }
                                }}
           ...
           {/* Object Controls - Moved from Right Panel to here */}
           {!isReCropping && (
           ...
           <div className="bg-gray-50 rounded-xl p-1 border border-gray-200 relative group flex items-center justify-center min-h-[400px] overflow-hidden select-none">
                ...
                >
                    {currentImage && (
                        <>
                            {isReCropping ? (
                                /* Re-Crop Mode: Show ReactCrop centered */
                                ...
                                /* Normal Mode: Draggable Image with Transform Controls */
                                <div 
                                    style={{
                                        position: 'absolute',
                                        top: styleTop,
                                ...
                                >
                                    ...
                                    {/* Selection Border (Visible on hover or drag) */}
    ...
    <div className="max-w-4xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-white rounded-2xl overflow-hidden border border-gray-200 shadow-lg">
             <div className="p-8 bg-gray-50 flex flex-col items-center justify-center border-r border-gray-200 relative">
                 {/* Visual checkboard for transparency */}
    ...
    <div className="min-h-screen py-8 px-4 flex flex-col overflow-x-hidden" style={{ background: 'var(--gradient-bg)' }}>
      <div className="max-w-6xl mx-auto flex-1 w-full">
        ...
        <div className="text-center mb-6 space-y-4">
          ...
          <p className="text-gray-600">這是一款易用圖片編輯工具，包含自由裁切、調整成常見社群或自訂尺寸，下載也可指定檔案大小喔</p>
